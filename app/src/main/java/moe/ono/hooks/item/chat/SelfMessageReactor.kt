...
class SelfMessageReactor : BaseClickableFunctionHookItem() {
    ...
    fun react(peerUid: Long, msgSeq: Long) {
        ...
        val faceIndex = ConfigManager.dGetInt(Constants.PrekCfgXXX + getItem(SelfMessageReactor::class.java).path, 355)


        // 利用发包回应，更通用
        // serviceType = 1 为回应, 2 则是取消回应
        val packetContent = """{"1": 36994,"2": 1,"4": {"2": $peerUid,"3": $msgSeq,"4": "$faceIndex","5": 1,"6": 0,"7": 0},"12": 1}"""
