...
class ProfileCardMenu : BaseSwitchFunctionHookItem() {
    ...
    companion object {
        ...
        private val statusMap = mapOf(
            0 to "离线", 1 to "在线", 3 to "离开", 4 to "隐身/离线",
            5 to "忙碌", 6 to "Q我吧", 7 to "请勿打扰", 1770497 to "恋爱中"
        )
        ...
        private fun collectCodes(node: Any?, out: MutableMap<UInt, String>) {
            when (node) {
                is JSONObject -> {
                    if (node.has("1") && node.has("2")) {
                        ...
                        if (codeLong > 0) {
                            ...
                            if (code in displayMap && code !in setOf(45160u, 45161u)) {
                                val v = parseValue(code, node.get("2"))
                    ...
                    val it = node.keys()
                    while (it.hasNext()) collectCodes(node.get(it.next()), out)
                }
        ...
        private fun parseValue(code: UInt, any: Any): String {
            return when (code) {
                ...
                101u, 103u -> (any as? JSONObject)?.optString("5").orEmpty() +  "640"
